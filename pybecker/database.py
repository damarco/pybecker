...
class Database:
    def __init__(self, path=None):
        if path:
            ...
            self.filename = os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "centronic-stick.db"
            )
    ...
    def migrate(self):
        try:
            ...
            self.old_file = os.path.join(
                os.path.dirname(os.path.realpath(__file__)), NUMBER_FILE
            )
    ...
    def create(self):
        ...
        c = self.conn.cursor()
        c.execute(
            "CREATE TABLE unit (code NVARCHAR(5), increment INTEGER(4), configured BIT, executed INTEGER, UNIQUE(code))"
        )
    ...
    def output(self):
        ...
        _LOGGER.info(
            "%-10s%-10s%-12s%-15s" % ("code", "increment", "configured", "last run")
        )
        _LOGGER.info(
            "%-10s%-18s%-12s%-15s"
            % ("code", "increment (hex)", "configured", "last run")
        )
        for line in res.fetchall():
            ...
            _LOGGER.info(
                "%-10s%-6s%-12s%-12s%-15s"
                % (line[0], line[1], "(0x" + hex4(line[1]) + ")", line[2], last_run)
            )
    ...
    def get_all_units(self):
        c = self.conn.cursor()
        res = c.execute(
            "SELECT code, increment, configured FROM unit WHERE configured = 1 ORDER BY code ASC"
        )
        result = []
    ...
    def add_unit(self, unit):
        ...
        c.execute(
            ...
            (unit[0], int(unit[1]), int(unit[2]), 0,),
        )
        self.conn.commit()
    ...
    def set_unit(self, unit, test=False):
        ...
        if len(unit[0]) < 5:
            ...
            c.execute(
                "UPDATE unit SET increment = ?, configured = ?, executed = ? WHERE code = ?",
                (unit[1], unit[2], last_run, unit[0],),
            )
